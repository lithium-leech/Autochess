...
var position: Vector2:
    get: return board.to_position(coordinates)
...
func enter(piece: Piece):
    ...
    if (object != null): piece.captured = object
    # Enter the terrain.
    if (terrain != null): terrain.enter(piece)
...
func exit(piece: Piece):
    ...
    if (object != piece): return
    # Exit the terrain.
    if (terrain != null): terrain.exit(piece)
...
func grab() -> GameObject:
    # Grab any equipment.
    # if (object is Piece and object.equipment != null): return object.unequip()
...
func add_object(_object: GameObject):
    ...
    if (!is_enterable(_object)): return
    # Update the object.
    _object.space = self
    # Add the object as a piece.
    if (_object is Piece):
        if (_object.is_player): board.player_pieces.append(_object)
        else: board.enemy_pieces.append(_object)
        object = _object;
...
func remove_object(_object: GameObject):
    ...
    if (_object is Piece):
        ...
        if (_object.is_player): board.player_pieces.erase(_object)
        else: board.enemy_pieces.erase(_object)
        object = null
    # Remove an equipment.
    elif (_object is Equipment and object is Piece):
        if (object.equipment != _object): return
        object.equipment = null;
    # Remove a terrain.
    elif (_object is Terrain):
        if (terrain != _object): return
        terrain = null;
    # Remove an object.
    else:
        if (object != _object): return
...
func is_enterable(_object: GameObject):
    if (object != null and _object is Equipment and object is Piece and object.equipment == null): return true
    elif (object != null): return false
    elif (terrain != null and _object is Terrain): return false
    elif (terrain != null and !terrain.is_enterable(_object)): return false
    else: return true
...
func has_piece() -> bool:
    ...
    elif (object is Piece): return true
    else: return false
...
func has_ally(player: bool) -> bool:
    if (!has_piece()): return false
    elif (object.is_player == player): return true
    else: return false
...
func has_enemy(player: bool) -> bool:
    if (!has_piece()): return false
    elif (object.is_player == player): return false
    else: return true
...
func has_capturable(piece: Piece):
    if (!has_enemy(piece.is_player)): return false
    else: return object.is_capturable(piece)
